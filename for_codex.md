...
- **Último micro-step concluído:** F2.A1.2 + F2.A3a — schema e normalização tri-state
  (este commit; `make check`: 619 passed, 2 skipped, 4 xfailed).
- **Micro-step corrente:** F2.A2 — distinguir header de tabela ausente de região vazia.
- **RETOME AQUI:** implementar `RawDocumentExtraction.tabela_encontrada` e fazer
  `_table_region` retornar `None` quando `_COLHDR` não casar; remover apenas os dois xfails de
  `tests/test_table_rules.py`. Não iniciar A4 antes do loop focado e do bloco tabular verdes.
...
- [ ] F2.A3b publicar `parse_times`/`parse_resolved` para reutilização futura no cockpit + commit
